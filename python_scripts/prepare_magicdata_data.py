...
def generate_group_wav_dict(batch_file):
    ...
    for group in ['wu', 'yue', 'chuan', 'dongbei']:
        ...
        group_selected_wavs = random_k_items(
            group_wavs, min_group_count, RANDOM_SEED)
        group_wav_dict[group] = group_selected_wavs

    # process guan separatly to include as much beijing&tianjin as possible
    hebei_quota = min_group_count - \
        len(prov_wav_dict['bei jing']) - len(prov_wav_dict['tian jin'])
    selected_hubei = random_k_items(
        prov_wav_dict['he bei'], hebei_quota, RANDOM_SEED)
    guan_selected_wavs = prov_wav_dict['bei jing'] + \
        prov_wav_dict['tian jin'] + selected_hubei
    random.seed(SHUFFLE_SEED)
...
generate_wavscp_utt2spk(dev_dict, wav_id_dict, 'dev_magicdata')
